...
function SidebarSubNavigation() {
	const [hasMask, setHasMask] = useState(false);
	const subNavRef = useRef();

	useEffect(() => {
		const subnavElement = subNavRef.current;
		const handleShowBoxShadow = e => {
			const scrollTop = e.target.scrollTop;
			setHasMask(scrollTop > 10);
		};
		subnavElement.addEventListener('scroll', handleShowBoxShadow);

		return () => subnavElement.removeEventListener('scroll', handleShowBoxShadow);
	}, []);
	return (
		<div
			className={clsx('sidebar__subnav', 'hide-on-mobile', {
				'has-mask': hasMask,
			})}
			ref={subNavRef}
		>
